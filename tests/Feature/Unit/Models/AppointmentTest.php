...
it('casts appointment_type to enum', function () {
    $appointment = Appointment::factory()->create(['appointment_type' => AppointmentType::Routine]);
...
it('casts appointment_date to date', function () {
    $appointment = Appointment::factory()->create(['appointment_date' => '2024-12-25']);
...
it('has exam_room relationship', function () {
    ...
    $appointment = Appointment::factory()->create(['exam_room_id' => $room->id]);
...
it('can scope by status', function () {
    Appointment::factory()->count(2)->create(['status' => AppointmentStatus::Scheduled]);
...
it('can scope by date range', function () {
    ...
    Appointment::factory()->create(['appointment_date' => $start->copy()->subDays(5)]);
    Appointment::factory()->create(['appointment_date' => $start->copy()->addDays(2)]);
    Appointment::factory()->create(['appointment_date' => $end->copy()->addDays(5)]);
